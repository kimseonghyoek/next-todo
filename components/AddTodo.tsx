  ...
  return (
    <Container>
      <div className="add-todo-header">
        ...
        <button type="button" className="add-todo-submit-button" onClick={()=>{}}>추가하기</button>
      </div>
      <div className="add-todo-color-wrapper">
        <div className="add-todo-color-list">
          {["red", "orange", "yellow", "green", "blue", "navy"].map((color, index) => (
            <button key={index} type="button" className={`bg-${color} add-todo-color-button ${color === selectedColor ? "add-todo-selected-color" : ""}`} onClick={() => setSelectedColor(color as TodoType["color"])}/>
          ))}
        </div>
        <BrushIcon/>
      </div>
